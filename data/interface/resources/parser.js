/* 
  Parts of this code is inspired from:
  https://huggingface.co/spaces/ibm-granite/granite-docling-258M-WebGPU/blob/main/parser.js
*/
...
export class DoclingConverter {
  ...
  convertToText(docling) {
    let text = this.cleanupMetadataTokens(docling);
    text = text
    .replace(/<paragraph>(.*?)<\/paragraph>/gs, '\n\n$1')
    .replace(/<section_header_level_[0-9]+>(.*?)<\/section_header_level_[0-9]+>/gs, '\n\n$1\n')
    .replace(/<list_item>(.*?)<\/list_item>/gs, '\n• $1')
    .replace(/<nl>/g, '\n')
